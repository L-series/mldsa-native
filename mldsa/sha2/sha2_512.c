...
/* Bitwise rotation macro */
static MLD_INLINE uint64_t ror64(uint64_t x, uint64_t n)
{
  return (x >> n) | (x << (64 - n));
}

/* Byte reversal for big-endian */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
#define rev8_be64(x) (x)
#else
static MLD_INLINE uint64_t rev8_be64(uint64_t x)
{
...
static void sha2_512_compress(void *v)
{
...
static void sha2_512_init_h0(sha2_512_t *sha, const uint8_t h0[64])
{
  mld_memcpy(sha->s, h0, 64);
  sha->i = 0;
  sha->len = 0;
}

static void sha2_512_init(sha2_512_t *sha)
{
...
static void sha2_384_init(sha2_512_t *sha)
{
...
static void sha2_512_224_init(sha2_512_t *sha)
{
...
static void sha2_512_256_init(sha2_512_t *sha)
{
...
/* SHA2-512 process input */

static void sha2_512_update(sha2_512_t *sha, const uint8_t *m, size_t m_sz)
{
  size_t l;
  ...
  if (m_sz < l)
  {
    mld_memcpy(mp + sha->i, m, m_sz);
    sha->i += m_sz;
    return;
  }
  if (sha->i > 0)
  {
    mld_memcpy(mp + sha->i, m, l);
  ...
  mld_memcpy(mp, m, m_sz);
  sha->i = m_sz;
}

/* perform final padding */

static void sha2_512_final_pad(sha2_512_t *sha)
{
  uint8_t *mp = (uint8_t *)&sha->s[8];
  uint64_t x;
  size_t i;
  ...
  if (i > 112)
  {
    mld_memset(mp + i, 0x00, 128 - i);
    sha2_512_compress(sha->s);
    i = 0;
  }
  mld_memset(mp + i, 0x00, 128 - i); /* clear rest */
...
static void sha2_512_final_len(sha2_512_t *sha, uint8_t *h, size_t h_sz)
{
  sha2_512_final_pad(sha);
  sha2_512_compress(sha->s);
  mld_memcpy(h, sha->s, h_sz);
}

static void sha2_512_final(sha2_512_t *sha, uint8_t *h)
{
  sha2_512_final_len(sha, h, 64);
}

static void sha2_384_final(sha2_512_t *sha, uint8_t *h)
{
  sha2_512_final_len(sha, h, 48);
}

static void sha2_512_224_final(sha2_512_t *sha, uint8_t *h)
{
  sha2_512_final_len(sha, h, 28);
}

static void sha2_512_256_final(sha2_512_t *sha, uint8_t *h)
{
  sha2_512_final_len(sha, h, 32);
}

/* SHA-512 public single-call interface */

void mld_sha2_512(uint8_t *h, const uint8_t *m, size_t m_sz)
{
  sha2_512_t sha;
  ...
  mld_zeroize(&sha, sizeof(sha));
}

/* SHA-384 public single-call interface */

void mld_sha2_384(uint8_t *h, const uint8_t *m, size_t m_sz)
{
  sha2_512_t sha;

  sha2_384_init(&sha);
  sha2_512_update(&sha, m, m_sz);
  sha2_384_final(&sha, h);
  mld_zeroize(&sha, sizeof(sha));
}

/* SHA-512/224 public single-call interface */

void mld_sha2_512_224(uint8_t *h, const uint8_t *m, size_t m_sz)
{
  sha2_512_t sha;

  sha2_512_224_init(&sha);
  sha2_512_update(&sha, m, m_sz);
  sha2_512_224_final(&sha, h);
  mld_zeroize(&sha, sizeof(sha));
}

/* SHA-512/256 public single-call interface */

void mld_sha2_512_256(uint8_t *h, const uint8_t *m, size_t m_sz)
{
  sha2_512_t sha;

  sha2_512_256_init(&sha);
  sha2_512_update(&sha, m, m_sz);
  sha2_512_256_final(&sha, h);
  mld_zeroize(&sha, sizeof(sha));
}
