...
static MLD_INLINE uint32_t ror32(uint32_t x, uint32_t n)
{
  return (x >> n) | (x << (32 - n));
}
...
#else
static MLD_INLINE uint32_t rev8_be32(uint32_t x)
{
...
static void sha2_256_compress(void *v)
{
...
{
  mld_memcpy(sha->s, h0, 32);
  sha->i = 0;
  sha->len = 0;
}

static void sha2_256_init(sha2_256_t *sha)
{
...
static void sha2_224_init(sha2_256_t *sha)
{
...
static void sha2_256_update(sha2_256_t *sha, const uint8_t *m, size_t m_sz)
{
  size_t l;
...
static void sha2_256_final_pad(sha2_256_t *sha)
{
  uint8_t *mp = (uint8_t *)&sha->s[8];
  uint64_t x;
  size_t i;
  ...
  if (i > 56)
  {
    mld_memset(mp + i, 0x00, 64 - i);
    sha2_256_compress(sha->s);
    i = 0;
  }
  mld_memset(mp + i, 0x00, 64 - i); /* clear rest */
...
static void sha2_256_final(sha2_256_t *sha, uint8_t *h)
{
  sha2_256_final_pad(sha);
  sha2_256_compress(sha->s);

  mld_memcpy(h, sha->s, 32);
}

static void sha2_224_final(sha2_256_t *sha, uint8_t *h)
{
  sha2_256_final_pad(sha);
  sha2_256_compress(sha->s);

  mld_memcpy(h, sha->s, 28);
}

/* SHA-256 public single-call interface */

void mld_sha2_256(uint8_t *h, const uint8_t *m, size_t m_sz)
{
  sha2_256_t sha;
...
/* SHA-224 public single-call interface */

void mld_sha2_224(uint8_t *h, const uint8_t *m, size_t m_sz)
{
  sha2_256_t sha;
