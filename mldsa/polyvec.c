                           ...
                           const uint8_t rho[MLDSA_SEEDBYTES])
{
  ...
  // initialize seed_ext, but unroll for simplicity
  memcpy(seed_ext[0], rho, MLDSA_SEEDBYTES);
  memcpy(seed_ext[1], rho, MLDSA_SEEDBYTES);
  memcpy(seed_ext[2], rho, MLDSA_SEEDBYTES);
  memcpy(seed_ext[3], rho, MLDSA_SEEDBYTES);
  ...
  {
    // Here's the original code with the 8617-wordaround...
    //    poly tmpvec[4];
    ...
    // Here it is with the work-around removed
