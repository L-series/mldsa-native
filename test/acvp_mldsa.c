...
#define SIGGEN_PREHASH_USAGE                                              \
  "acvp_mldsa{lvl} sigGenPreHash message=HEX context=HEX rng=HEX sk=HEX " \
  "hashAlg=STRING"
#define SIGVER_PREHASH_USAGE                                             \
  "acvp_mldsa{lvl} sigVerPreHash message=HEX context=HEX signature=HEX " \
  "pk=HEX "                                                              \
  "hashAlg=STRING"
...
static int acvp_mldsa_sigGenPreHash_AFT(
    const unsigned char *m, size_t mlen, const unsigned char *context,
    ...
    const unsigned char sk[CRYPTO_SECRETKEYBYTES], const char *hashAlg)
{
  ...
  if (crypto_sign_signature_pre_hash(signature, &siglen, m, mlen, context,
                                     ctxlen, rng, sk,
...
{
  return crypto_sign_verify_pre_hash(signature, CRYPTO_BYTES, m, mlen, context,
                                     ctxlen, pk, str_to_hash_alg(hashAlg));
}
...
int main(int argc, char *argv[])
{
  ...
  switch (mode)
  {
    ...
    case sigGenPreHash:
    {
      ...
      /* Call function under test */
      return acvp_mldsa_sigGenPreHash_AFT(message, mlen, context, ctxlen, rng,
                                          sk, hashAlg);
    }
    ...
    case sigVerPreHash:
    {
      ...
      /* Call function under test */
      return acvp_mldsa_sigVerPreHash_AFT(message, mlen, context, ctxlen,
                                          signature, pk, hashAlg);
    }
  }
